...
import { COMPLETION_V3_AUTH_ENV, authorizeCompletionV3Run,
...
const outputDir = path.resolve(root, outputArg?.slice("--output-dir=".length)
  ?? ".cache/eval/unit-play-guidance-completion-v3/formal");
...
const authorization = authorizeCompletionV3Run({ config, preflightResult, transportMode: "real_provider",
  ...
  environmentAuthorization: process.env[COMPLETION_V3_AUTH_ENV], apiKey: process.env.OPENAI_API_KEY,
...
const checkpointPath = path.join(outputDir, "checkpoints.v3.jsonl");
const onCheckpoint = async ({ completedAgentRuns, run }) => appendFile(checkpointPath,
  `${JSON.stringify({ schemaVersion: "unit-play-guidance-completion-checkpoint.v3", completedAgentRuns,
...
for (const [name, value] of [["result.v3.json", output.result], ["review-packet.v3.json", output.review.packet],
