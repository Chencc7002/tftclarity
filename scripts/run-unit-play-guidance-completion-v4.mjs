...
import { COMPLETION_V4_AUTH_ENV, authorizeCompletionV4Run,
...
const outputDir = path.resolve(root, outputArg?.slice("--output-dir=".length)
  ?? ".cache/eval/unit-play-guidance-completion-v4/formal");
...
const authorization = authorizeCompletionV4Run({ config, preflightResult, transportMode: "real_provider",
  ...
  environmentAuthorization: process.env[COMPLETION_V4_AUTH_ENV], apiKey: process.env.OPENAI_API_KEY,
...
const checkpointPath = path.join(outputDir, "checkpoints.v4.jsonl");
const onCheckpoint = async ({ completedAgentRuns, run }) => appendFile(checkpointPath,
  `${JSON.stringify({ schemaVersion: "unit-play-guidance-completion-checkpoint.v4", completedAgentRuns,
...
for (const [name, value] of [["result.v4.json", output.result], ["review-packet.v4.json", output.review.packet],
