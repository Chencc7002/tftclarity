...
import { COMPLETION_V6_AUTH_ENV, authorizeCompletionV6Run,
...
const outputDir = path.resolve(root, outputArg?.slice("--output-dir=".length)
  ?? ".cache/eval/unit-play-guidance-completion-v6/formal");
...
const authorization = authorizeCompletionV6Run({ config, preflightResult, transportMode: "real_provider",
  ...
  environmentAuthorization: process.env[COMPLETION_V6_AUTH_ENV], apiKey: process.env.OPENAI_API_KEY,
...
const checkpointPath = path.join(outputDir, "checkpoints.v6.jsonl");
const onCheckpoint = async ({ completedAgentRuns, run }) => appendFile(checkpointPath,
  `${JSON.stringify({ schemaVersion: "unit-play-guidance-completion-checkpoint.v6", completedAgentRuns,
...
for (const [name, value] of [["result.v6.json", output.result], ["review-packet.v6.json", output.review.packet],
