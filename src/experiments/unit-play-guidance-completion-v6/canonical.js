...
import { FORWARD_EXPECTED_TOOL_SEQUENCE,
...
import { buildCompletionV6Plan, COMPLETION_V6_EXPERIMENT_ID } from "./preflight.js";
import { COMPLETION_V6_REVIEW_FACETS } from "./review-facets.js";

export const COMPLETION_V6_AUTH_SCHEMA = "unit-play-guidance-completion-provider-authorization.v6";
export const COMPLETION_V6_AUTH_ENV = "UNIT_PLAY_GUIDANCE_COMPLETION_PROVIDER_AUTHORIZED";
export const COMPLETION_V6_LIMITS = Object.freeze({ providerHttpRequestHardCap: 1_000,
...
export function createCompletionV6Fuse() {
  ...
  return {
    beforeRequest() {
      if (exhaustedReason) fail(`completion v6 fuse is open: ${exhaustedReason}`, "budget_failure");
      if (providerHttpRequests + 1 > COMPLETION_V6_LIMITS.providerHttpRequestHardCap) {
        fail("completion v6 HTTP-request hard cap reached", "budget_failure");
      }
    ...
    snapshot: () => ({ limits: COMPLETION_V6_LIMITS, providerHttpRequests, totalTokens,
      responsesWithoutUsage, exhausted: exhaustedReason !== null, exhaustedReason })
  };
}

export function authorizeCompletionV6Run({ config, preflightResult, transportMode = "real_provider",
  ...
  if (environmentAuthorization !== "1") failures.push(`${COMPLETION_V6_AUTH_ENV} must equal 1`);
  ...
  if (providerAuthorization?.schemaVersion !== COMPLETION_V6_AUTH_SCHEMA
    || providerAuthorization?.experimentId !== COMPLETION_V6_EXPERIMENT_ID
  ...
  if (failures.length) throw taggedError(`completion v6 unlock denied: ${failures.join("; ")}`, "authorization_failed");
...
function labelTemplates(packet) {
  return ["reviewer-1", "reviewer-2"].map((reviewerId) => ({
    ...
    experimentId: COMPLETION_V6_EXPERIMENT_ID, reviewerId, packetSha256: sha256(packet),
...
export async function runCompletionV6Experiment({ config, corpus, observations, preflightResult,
  authorization, fetchImpl, onCheckpoint = null } = {}) {
  if (!authorization || !["fake_test", "real_provider"].includes(authorization.transportMode)) {
    throw taggedError("completion v6 requires authorization proof", "authorization_failed");
  }
  ...
  if (preflightResult?.status !== "passed") throw taggedError("completion v6 preflight failed", "frozen_input_drift");
  if (sha256(corpus) !== config.frozen.sourceCorpusNormalizedSha256
    ...
    || sha256(JSON.stringify(UNIT_PLAY_GUIDANCE_SKILL_V1_5_11)) !== config.frozen.candidateSkillSha256) {
    throw taggedError("completion v6 frozen input drift", "frozen_input_drift");
  }
  if (config?.runtimeAffordance?.unitPlayFixedCardCompletionAffordance !== true
    || config.runtimeAffordance.unitPlayFixedCardCount !== 2
    || config.runtimeAffordance.unitPlayInputLanguageGuard !== true) {
    throw taggedError("completion v6 runtime affordance is not frozen", "frozen_input_drift");
  }
  const plan = buildCompletionV6Plan(config, corpus);
  if (plan.orderSha256 !== config.execution.orderSha256) throw taggedError("completion v6 order drift", "frozen_input_drift");
  const toolRegistry = new ToolRegistry(createStructuredToolDefinitions());
  const fuse = createCompletionV6Fuse();
  ...
  for (const planned of plan.runs) {
    ...
    const run = await runForwardCanonicalArm({ arm: "B", pair: { pairId: planned.runId,
      ...
      candidateSkill: UNIT_PLAY_GUIDANCE_SKILL_V1_5_11,
      runContextOptions: config.runtimeAffordance });
    runs.push(run);
  ...
  const rawReview = buildForwardBlindedReviewArtifacts(validRuns,
    createHash("sha256").update(`${implementationSeed(authorization)}\0completion-v6`).digest("hex"));
  const packet = { ...rawReview.packet, schemaVersion: "unit-play-guidance-completion-review-packet.v6",
    experimentId: COMPLETION_V6_EXPERIMENT_ID, adaptiveCandidateOnly: true };
  const key = { ...rawReview.key, schemaVersion: "unit-play-guidance-completion-review-key.v6",
    experimentId: COMPLETION_V6_EXPERIMENT_ID };
  return { result: { schemaVersion: "unit-play-guidance-completion-result.v6",
    experimentId: COMPLETION_V6_EXPERIMENT_ID, status: passed ? "awaiting_independent_review" : "inconclusive",
