...
import { UNIT_PLAY_GUIDANCE_SKILL, UNIT_PLAY_GUIDANCE_SKILL_V1_5_11 } from "../../skills/definitions/unit-play-guidance.js";
...
export const COMPLETION_V6_EXPERIMENT_ID = "unit-play-guidance-completion.2026-09-02.v6";
export const COMPLETION_V6_SCHEMA = "unit-play-guidance-completion-preflight.v6";

export function buildCompletionV6Plan(config, corpus) {
...
function renderedCandidateHash(evalCase, toolRegistry) {
  ...
  const skillRegistry = new SkillRegistry({ definitions: [UNIT_PLAY_GUIDANCE_SKILL_V1_5_11], toolRegistry });
  const selection = matchSkill(taskFrame, skillRegistry);
  if (selection.status !== "selected") return { selection, hash: null };
  const skillContext = buildSkillContext({ skill: UNIT_PLAY_GUIDANCE_SKILL_V1_5_11, selection, taskFrame,
  ...
  return { selection, hash: sha256(JSON.stringify({ schemaVersion: "unit-play-browser-candidate.v1",
    contentHash: sha256(JSON.stringify(UNIT_PLAY_GUIDANCE_SKILL_V1_5_11)), skillContext })) };
}

export function runCompletionV6Preflight({ config, corpus, observations, sourcePreflight }) {
  ...
  const gates = {
    configIdentity: config?.schemaVersion === "unit-play-guidance-completion-config.v6"
      && config.experimentId === COMPLETION_V6_EXPERIMENT_ID,
    adaptiveDisclosure: config?.adaptiveDisclosure?.priorAttemptsObserved === true
      && config.adaptiveDisclosure.answerContentUsedForTuning === true
    ...
    candidateSkill: UNIT_PLAY_GUIDANCE_SKILL_V1_5_11.version === "1.5.11"
    ...
    runtimeAffordance: config?.runtimeAffordance?.unitPlayFixedCardCompletionAffordance === true
  ...
  return { schemaVersion: COMPLETION_V6_SCHEMA, experimentId: COMPLETION_V6_EXPERIMENT_ID,
    ...
    hashes: { sourceCorpus: sha256(corpus), sourceObservations: sha256(observations),
      candidateSkill: sha256(JSON.stringify(UNIT_PLAY_GUIDANCE_SKILL_V1_5_11)),
