...
import { COMPLETION_V5_EXPERIMENT_ID } from "./preflight.js";

export const COMPLETION_V5_REVIEW_RATINGS = Object.freeze([
  "pass",
  "partial",
  "fail",
  "not_applicable"
...
const PACKET_SCHEMA = "unit-play-guidance-completion-review-packet.v5";
const LABEL_SCHEMA = "unit-play-guidance-completion-independent-labels.v5";
const ADJUDICATION_SCHEMA = "unit-play-guidance-completion-adjudication.v5";
const RESULT_SCHEMA = "unit-play-guidance-completion-independent-review-result.v5";
const ratingSet = new Set(COMPLETION_V5_REVIEW_RATINGS);
const facetSet = new Set(FORWARD_REVIEW_FACETS);
...
function validatePacket(packet) {
  ...
  if (packet.schemaVersion !== PACKET_SCHEMA
    || packet.experimentId !== COMPLETION_V5_EXPERIMENT_ID
    || packet.adaptiveCandidateOnly !== true
    || !Array.isArray(packet.entries)
    || packet.entries.length === 0) {
    throw new TypeError("Unsupported completion v5 review packet");
  }
  ...
  for (const entry of packet.entries) {
    if (typeof entry?.outputId !== "string" || !entry.outputId
      ...
      throw new TypeError("Invalid or duplicate completion v5 packet entry");
    }
...
function validateReviewerLabels(packet, reviewerLabels) {
  ...
  const expectedKeys = new Set(packet.entries.flatMap((entry) => (
    FORWARD_REVIEW_FACETS.map((facetId) => labelKey({ outputId: entry.outputId, facetId }))
  )));
  const normalized = reviewerLabels.map((document) => {
    ...
    if (document.schemaVersion !== LABEL_SCHEMA
      || document.experimentId !== COMPLETION_V5_EXPERIMENT_ID
...
export function inspectCompletionV5IndependentReview({ packet, reviewerLabels } = {}) {
  ...
  if (labelsComplete) {
    for (const entry of packet.entries) {
      for (const facetId of FORWARD_REVIEW_FACETS) {
  ...
  return {
    schemaVersion: "unit-play-guidance-completion-review-progress.v5",
...
export function createCompletionV5AdjudicationTemplate({ packet, reviewerLabels } = {}) {
...
function validateAdjudication(progress, reviewerLabels, adjudication) {
  ...
  if (adjudication.schemaVersion !== ADJUDICATION_SCHEMA
    || adjudication.experimentId !== COMPLETION_V5_EXPERIMENT_ID
    ...
    throw new TypeError("Invalid completion v5 adjudication document");
  }
...
export function finalizeCompletionV5IndependentReview({ packet, reviewerLabels, adjudication } = {}) {
  const progress = inspectCompletionV5IndependentReview({ packet, reviewerLabels });
  if (progress.status === "awaiting_independent_review") {
  ...
  const ratingCounts = Object.fromEntries(COMPLETION_V5_REVIEW_RATINGS.map((rating) => [rating,
    ratings.filter((entry) => entry.rating === rating).length]));
  const facetSummary = Object.fromEntries(FORWARD_REVIEW_FACETS.map((facetId) => [facetId,
    Object.fromEntries(COMPLETION_V5_REVIEW_RATINGS.map((rating) => [rating,
