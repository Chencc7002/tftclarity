...
import { UNIT_PLAY_GUIDANCE_SKILL, UNIT_PLAY_GUIDANCE_SKILL_V1_5_9 } from "../../skills/definitions/unit-play-guidance.js";
...
export const COMPLETION_V5_EXPERIMENT_ID = "unit-play-guidance-completion.2026-09-01.v5";
export const COMPLETION_V5_SCHEMA = "unit-play-guidance-completion-preflight.v5";

export function buildCompletionV5Plan(config, corpus) {
...
function renderedCandidateHash(evalCase, toolRegistry) {
  ...
  const skillRegistry = new SkillRegistry({ definitions: [UNIT_PLAY_GUIDANCE_SKILL_V1_5_9], toolRegistry });
  const selection = matchSkill(taskFrame, skillRegistry);
  if (selection.status !== "selected") return { selection, hash: null };
  const skillContext = buildSkillContext({ skill: UNIT_PLAY_GUIDANCE_SKILL_V1_5_9, selection, taskFrame,
    runtimeAvailableTools: UNIT_PLAY_GUIDANCE_SKILL_V1_5_9.allowedTools });
  return { selection, hash: sha256(JSON.stringify({ schemaVersion: "unit-play-browser-candidate.v1",
    contentHash: sha256(JSON.stringify(UNIT_PLAY_GUIDANCE_SKILL_V1_5_9)), skillContext })) };
}

export function runCompletionV5Preflight({ config, corpus, observations, sourcePreflight }) {
  ...
  const gates = {
    configIdentity: config?.schemaVersion === "unit-play-guidance-completion-config.v5"
      && config.experimentId === COMPLETION_V5_EXPERIMENT_ID,
    ...
    candidateSkill: UNIT_PLAY_GUIDANCE_SKILL_V1_5_9.version === "1.5.9"
  ...
  return { schemaVersion: COMPLETION_V5_SCHEMA, experimentId: COMPLETION_V5_EXPERIMENT_ID,
    ...
    hashes: { sourceCorpus: sha256(corpus), sourceObservations: sha256(observations),
      candidateSkill: sha256(JSON.stringify(UNIT_PLAY_GUIDANCE_SKILL_V1_5_9)),
