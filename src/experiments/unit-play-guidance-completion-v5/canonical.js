...
import { buildCompletionV5Plan, COMPLETION_V5_EXPERIMENT_ID } from "./preflight.js";

export const COMPLETION_V5_AUTH_SCHEMA = "unit-play-guidance-completion-provider-authorization.v5";
export const COMPLETION_V5_AUTH_ENV = "UNIT_PLAY_GUIDANCE_COMPLETION_PROVIDER_AUTHORIZED";
export const COMPLETION_V5_LIMITS = Object.freeze({ providerHttpRequestHardCap: 1_000,
...
export function createCompletionV5Fuse() {
  ...
  return {
    beforeRequest() {
      if (exhaustedReason) fail(`completion v5 fuse is open: ${exhaustedReason}`, "budget_failure");
      if (providerHttpRequests + 1 > COMPLETION_V5_LIMITS.providerHttpRequestHardCap) {
        fail("completion v5 HTTP-request hard cap reached", "budget_failure");
      }
    ...
    snapshot: () => ({ limits: COMPLETION_V5_LIMITS, providerHttpRequests, totalTokens,
      responsesWithoutUsage, exhausted: exhaustedReason !== null, exhaustedReason })
  };
}

export function authorizeCompletionV5Run({ config, preflightResult, transportMode = "real_provider",
  ...
  if (environmentAuthorization !== "1") failures.push(`${COMPLETION_V5_AUTH_ENV} must equal 1`);
  ...
  if (providerAuthorization?.schemaVersion !== COMPLETION_V5_AUTH_SCHEMA
    || providerAuthorization?.experimentId !== COMPLETION_V5_EXPERIMENT_ID
  ...
  if (failures.length) throw taggedError(`completion v5 unlock denied: ${failures.join("; ")}`, "authorization_failed");
...
function labelTemplates(packet) {
  return ["reviewer-1", "reviewer-2"].map((reviewerId) => ({
    schemaVersion: "unit-play-guidance-completion-independent-labels.v5",
    experimentId: COMPLETION_V5_EXPERIMENT_ID, reviewerId, packetSha256: sha256(packet),
...
export async function runCompletionV5Experiment({ config, corpus, observations, preflightResult,
  authorization, fetchImpl, onCheckpoint = null } = {}) {
  if (!authorization || !["fake_test", "real_provider"].includes(authorization.transportMode)) {
    throw taggedError("completion v5 requires authorization proof", "authorization_failed");
  }
  ...
  if (preflightResult?.status !== "passed") throw taggedError("completion v5 preflight failed", "frozen_input_drift");
  if (sha256(corpus) !== config.frozen.sourceCorpusNormalizedSha256
    ...
    throw taggedError("completion v5 frozen input drift", "frozen_input_drift");
  }
  if (config?.runtimeAffordance?.unitPlayFixedCardCompletionAffordance !== true
    || config.runtimeAffordance.unitPlayFixedCardCount !== 2) {
    throw taggedError("completion v5 runtime affordance is not frozen", "frozen_input_drift");
  }
  const plan = buildCompletionV5Plan(config, corpus);
  if (plan.orderSha256 !== config.execution.orderSha256) throw taggedError("completion v5 order drift", "frozen_input_drift");
  const toolRegistry = new ToolRegistry(createStructuredToolDefinitions());
  const fuse = createCompletionV5Fuse();
  ...
  const rawReview = buildForwardBlindedReviewArtifacts(validRuns,
    createHash("sha256").update(`${implementationSeed(authorization)}\0completion-v5`).digest("hex"));
  const packet = { ...rawReview.packet, schemaVersion: "unit-play-guidance-completion-review-packet.v5",
    experimentId: COMPLETION_V5_EXPERIMENT_ID, adaptiveCandidateOnly: true };
  const key = { ...rawReview.key, schemaVersion: "unit-play-guidance-completion-review-key.v5",
    experimentId: COMPLETION_V5_EXPERIMENT_ID };
  return { result: { schemaVersion: "unit-play-guidance-completion-result.v5",
    experimentId: COMPLETION_V5_EXPERIMENT_ID, status: passed ? "awaiting_independent_review" : "inconclusive",
