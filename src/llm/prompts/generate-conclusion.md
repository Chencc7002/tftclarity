...
10. 对 `unit_item_rankings`，`recommendations` 与前端实际展示的候选完全一致，必须综合分析全部候选，不能只复述 rank=1：
    - 当 `itemRankingContext.mixedCategoryRanking=true` 时，这是跨装备类别混榜：先比较候选在自身类别内的动态样本等级，同等级再比较经收缩的表现分。必须提示不同类别获取条件不同，榜单只表达携带后的描述性表现。不得把绝对样本量直接跨类别比较，也不得将样本量说成表现分加分项。
    - `itemRankingContext.specialAveragePlacementOnly=true` 仅用于兼容旧证据；遇到旧证据时仍严格遵守其中已给出的平均名次排序。
