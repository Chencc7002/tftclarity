...
test("builds item catalog categories from captured MetaTFT items response", () => {
  ...
  assert.equal(byApiName.get("TFT5_Item_BlueBuffRadiant").zhName, "光明蓝霸符");
  ...
  assert.equal(byApiName.get("TFT17_Item_FavoredEmblemItem").shortName, "Favored Emblem");
  ...
  assert.equal(byApiName.get("TFT17_AnimaSquadItem_Tier2_BattleBunnyCrossbow").zhName, "幻灵战斗兔弩");
  ...
  assert.equal(byApiName.get("TFT17_Item_PsyOps_DroneMod_Radiant").zhName, "光明灵能无人机改件");
