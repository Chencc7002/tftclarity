...
import { COMPLETION_V6_AUTH_SCHEMA, authorizeCompletionV6Run,
  runCompletionV6Experiment } from "../src/experiments/unit-play-guidance-completion-v6/canonical.js";
import { createForwardScriptedTransport } from "../src/experiments/unit-play-guidance-forward/scripted-transport.js";
import { preferredAnswerLanguage } from "../src/react/termination-policy.js";

const load = async (path) => JSON.parse(await readFile(new URL(path, import.meta.url), "utf8"));
const raw = async (path) => createHash("sha256").update(await readFile(new URL(path, import.meta.url))).digest("hex");
const config = await load("../eval/skills/unit-play-guidance-completion-v6/config.v6.json");
...
const preflightResult = await load("../eval/skills/unit-play-guidance-completion-v6/preflight-result.v6.json");
const manifest = await load("../eval/skills/unit-play-guidance-completion-v6/run-manifest.v6.json");

test("adaptive v6 zero-call preflight freezes candidate-only reliability without an efficacy claim", () => {
  const current = runCompletionV6Preflight({ config, corpus, observations, sourcePreflight });
...
test("v6 authorization binds the adaptive scope, commit, config, and 1000-request limit", () => {
  const commit = "d".repeat(40);
  const providerAuthorization = { schemaVersion: COMPLETION_V6_AUTH_SCHEMA,
  ...
  const authorization = authorizeCompletionV6Run({ config, preflightResult, transportMode: "real_provider",
  ...
  assert.throws(() => authorizeCompletionV6Run({ config, preflightResult, transportMode: "real_provider",
    cliAuthorized: true, environmentAuthorization: "1", apiKey: "test", worktreeClean: true,
    implementationCommitSha: "e".repeat(40), providerAuthorization }), /binding mismatch/u);
});

test("v6 scripted transport completes 90 candidate runs and emits review artifacts with zero Provider calls", async () => {
  ...
  assert.equal(output.review.labels.every((entry) => entry.labels.length === 630), true);
  assert.equal(output.review.packet.entries.every((entry) => (
    preferredAnswerLanguage(entry.answer) === preferredAnswerLanguage(entry.input)
  )), true);
  assert.equal(transport.snapshot().requests, 810);
});

test("v6 stops early once reliability gates are mathematically unreachable", async () => {
  const earlyFallback = async () => {
    const body = { choices: [{ message: { content: JSON.stringify({ schemaVersion: "react-action.v1",
      ...
      narrative: null }) } }], model: "deepseek-v6-flash-test", system_fingerprint: "completion-v6-test",
...
test("v6 manifest pins the adaptive implementation and keeps Provider and production locked", async () => {
  ...
  assert.equal(await raw("../src/experiments/unit-play-guidance-completion-v6/preflight.js"),
    manifest.implementation.preflightRawSha256);
  assert.equal(await raw("../src/experiments/unit-play-guidance-completion-v6/canonical.js"),
    manifest.implementation.canonicalRawSha256);
  assert.equal(await raw("../scripts/run-unit-play-guidance-completion-v6.mjs"),
    manifest.implementation.formalRunnerRawSha256);
  assert.equal(await raw("../scripts/run-unit-play-guidance-completion-v6-dry-run.mjs"),
