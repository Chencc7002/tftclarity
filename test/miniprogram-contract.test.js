...
test("mini program patch notes use the numeric-only traceable chain", () => {
  ...
  assert.equal(patch.history.length, 4);
  ...
  assert.equal(view.cards[0].id, "18.3-release-2026-09-23");
  assert.match(view.cards[0].subtitle, /18\.2-hotfix-2026-09-14/u);
  assert.equal(view.cards[1].id, "18.2-hotfix-2026-09-14");
  assert.match(view.cards[1].subtitle, /18\.2-release-2026-09-09/u);
  assert.equal(view.cards[2].id, "18.2-release-2026-09-09");
  assert.match(view.cards[2].subtitle, /18\.1-balance-2026-08-31/u);
  assert.equal(view.cards[3].id, "18.1-balance-2026-08-31");
  assert.match(view.cards[3].description, /增强｜阿木木/u);
  assert.match(view.cards[2].description, /增强｜阿狸/u);
  assert.match(view.cards[2].description, /削弱｜阿狸/u);
});
