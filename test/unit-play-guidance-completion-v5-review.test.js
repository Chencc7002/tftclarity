...
import { FORWARD_REVIEW_FACETS } from "../src/experiments/unit-play-guidance-forward/canonical.js";
import { sha256 } from "../src/experiments/unit-play-guidance-control/content.js";
import { COMPLETION_V5_EXPERIMENT_ID } from "../src/experiments/unit-play-guidance-completion-v5/preflight.js";
...
const packet = {
  schemaVersion: "unit-play-guidance-completion-review-packet.v5",
...
test("v5 review progress preserves incomplete independent labels without semantic auto-scoring", () => {
  const progress = inspectCompletionV5IndependentReview({ packet,
    reviewerLabels: [reviewer("reviewer-1"), reviewer("reviewer-2")] });
  assert.equal(progress.status, "awaiting_independent_review");
  assert.deepEqual(progress.reviewerProgress.map((entry) => entry.pendingRatings), [12, 12]);
  assert.equal(progress.productionAuthorization, false);
  assert.equal(progress.disagreements.length, 0);
});

test("v5 review rejects duplicate, missing, foreign, and non-independent labels", () => {
  const one = reviewer("reviewer-1", "pass");
  const two = reviewer("reviewer-2", "pass");
  one.labels.pop();
  assert.throws(() => inspectCompletionV5IndependentReview({ packet, reviewerLabels: [one, two] }), /Missing labels/u);
  const duplicate = reviewer("reviewer-1", "pass");
  duplicate.labels[1] = structuredClone(duplicate.labels[0]);
  assert.throws(() => inspectCompletionV5IndependentReview({ packet, reviewerLabels: [duplicate, two] }), /duplicate/u);
  const sameIdentity = reviewer("reviewer-1", "pass");
  assert.throws(() => inspectCompletionV5IndependentReview({ packet,
    reviewerLabels: [reviewer("reviewer-1", "pass"), sameIdentity] }), /distinct/u);
});

test("v5 adjudication is prepared only after both reviewers finish and contains disagreements only", () => {
  const one = reviewer("reviewer-1", "pass");
  const two = reviewer("reviewer-2", "pass");
  two.labels[0].rating = "partial";
  const progress = inspectCompletionV5IndependentReview({ packet, reviewerLabels: [one, two] });
  assert.equal(progress.status, "awaiting_adjudication");
  assert.equal(progress.agreements, 11);
  assert.equal(progress.disagreements.length, 1);
  const adjudication = createCompletionV5AdjudicationTemplate({ packet, reviewerLabels: [one, two] });
  assert.equal(adjudication.entries.length, 1);
  assert.equal(adjudication.entries[0].rating, null);
  assert.throws(() => createCompletionV5AdjudicationTemplate({ packet,
    reviewerLabels: [reviewer("reviewer-1"), reviewer("reviewer-2")] }), /must be complete/u);
});

test("v5 finalization requires substantive adjudication and reports quality without authorizing production", () => {
  const one = reviewer("reviewer-1", "pass");
  const two = reviewer("reviewer-2", "pass");
  ...
  assert.throws(() => finalizeCompletionV5IndependentReview({ packet, reviewerLabels, adjudication }), /incomplete/u);
  ...
  assert.equal(result.totalFacetRatings, 12);
  assert.equal(result.reviewerAgreements, 11);
  assert.equal(result.adjudicatedDisagreements, 1);
  assert.deepEqual(result.ratingCounts, { pass: 11, partial: 1, fail: 0, not_applicable: 0 });
