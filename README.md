...
tftclarity 是面向《云顶之弈》的对话式数据分析与复盘工具。它把“霞有羊刀后两件怎么补”“当前有哪些阵容正在上升”这类问题转换为受控的结构化查询，结合 MetaTFT 统计、版本快照、近期对局、攻略知识和可选的 AI 解读，返回结论、样本规模、风险提示与证据来源。
...
- **快速得到结论**：不必在完整榜单中反复筛选，系统直接组织首选方案、备选方案、关键指标和下一步建议。
- **中文俗称与连续追问**：可以直接说“羊刀、无尽、巨杀、石像鬼”，后续追问会继承英雄、阵容和筛选条件。
- **查询、复盘和教学一体化**：既能查装备与阵容，也能查看近期终局棋盘、高手样本和攻略知识。
...
职业与自定义玩家样本支持阵容频率、玩家覆盖、平均名次、前四率、登顶率、老八率、代表棋盘和装备热度。AI 教学与复盘只基于已经取得的结构化证据，不把有限玩家样本冒充全服 Meta。
...
Pool 是“战绩、Pool 与复盘”中的进阶能力，用于把用户自己选择的玩家组织成可维护的小数据集；它不是产品的唯一入口，也不替代装备、阵容、百科和个人复盘功能。每位访客最多管理 2 个 Pool，每组 1–15 个角色。
