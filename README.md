...
| 常规数据网站 | tftclarity |
| --- | --- |
| 依赖准确名称和多级筛选器 | 支持中文俗称和自然语言描述 |
| 主要展示数据，由用户自行判断 | 汇总关键指标并给出首选与备选 |
| 每次查询通常相互独立 | 支持多轮条件继承和修改 |
| 高胜率结果可能受低样本影响 | 推荐排序会结合样本量和适用范围 |
| 查看阵容内不同棋子时需要重复筛选 | 支持阵容内棋子快捷查询和返回导航 |
| AI 总结可能脱离数据 | AI 解读必须通过结构化证据校验 |

tftclarity 不以替代完整的数据网站为目标。如果需要浏览全部原始榜单或进行大范围人工筛选，传统数据网站仍然更合适；本项目更侧重快速查询、连续决策和证据约束下的推荐。
