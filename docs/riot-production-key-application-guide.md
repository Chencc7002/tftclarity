...
- [x] Privacy Policy 已在仓库实现为 `/privacy`；待部署后从公网复核。
- [x] Terms of Service 已在仓库实现为 `/terms`；待部署后从公网复核。
...
- [ ] `https://tftclarity.cn/privacy` 可访问。
- [ ] `https://tftclarity.cn/terms` 可访问。
- [x] 英文产品说明初稿已完成。
- [ ] 英文产品说明已由运营者最终审核。
- [ ] 主要用户流程截图或视频已准备。
- [ ] `longyuyanchen@gmail.com` 已配置并完成收信测试。
- [ ] 当前数据源和迁移计划已如实说明。
- [ ] 所需 Riot API 和目标区域已列出。
- [ ] 实时、对手侦察和隐藏玩家边界已声明。
