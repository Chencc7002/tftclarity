...
| 自动化测试 | 2026-07-20 执行 436 项：421 通过、15 跳过、0 失败 | 已达标 |
| Privacy Policy | `/privacy` 页面、无扩展名路由和公开入口已在仓库实现 | 待部署复核 |
| Terms of Service | `/terms` 页面、无扩展名路由和公开入口已在仓库实现 | 待部署复核 |
...
```text
GET https://tftclarity.cn/         → 200
GET https://tftclarity.cn/privacy → 上次公网核验为 404；代码已实现，待重新部署与复核
GET https://tftclarity.cn/terms   → 上次公网核验为 404；代码已实现，待重新部署与复核
```

首页当前可正常加载，固定页脚和设置面板中已经存在 Riot 独立项目声明，但尚未出现 Privacy Policy 与 Terms of Service 链接。最新 `origin/main` 也尚未包含对应静态页面与无扩展名路由，因此两页不能视为已上线。
