...
`productionControl: false`，冻结 config 不再为正式运行而修改。用户明确授权后，另行
生成不提交仓库的一次性授权文件；它绑定准确 commit、冻结 config 哈希、180 次 Agent
run 上限、Provider 主机和模型。正式入口还会分别校验命令行开关、环境变量、凭据、
...
取消总 token 上限后的第二次尝试绑定提交
`842579f2b3a16944770f02a5af96f4e8db7efdfc`，在 26 个 arm 后停止：207 次 Provider
请求、2449197 token，覆盖 5 个 case。A 臂 13 次原生完成为 0；B 臂 13/13 都完成固定
8 步工具序列，但只有 4 次通过原生完成审计。已有 4 个完整 case 为 0 个有效 pair，数学上
无法再达到 27/30 case 门槛，因此没有继续消耗。该尝试同样只有检查点，没有完整结果或
盲包。

有界 action trace 显示 B 的主要剩余问题是两张来源卡已取齐后仍尝试第三次阵容或重复
战术查询。新建 1.5.8 候选只修这一完成条件：首次来源候选集合固定，逐卡处理一次后立即
finish；finish 被拒绝时只修格式或引用，不用新检索代替答案修正。它不改工具、权限、
Evidence、装备、阵容、站位或生产默认。后续评估必须声明为看过上述诊断后的自适应新实验，
不能把两次中止尝试拼成正式效果结果。

1.5.8 内容 SHA256：
`7d2e6ca88c8c8300ab6bdbf28d3a5a567198993c489e0d3a73984a29d48ff405`。
