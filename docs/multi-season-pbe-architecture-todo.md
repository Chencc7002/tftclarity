...
> 状态：阶段 1-10 已完成；S18 已提升为正式服默认空间，S17 并行保留，S18 PBE 已从公开注册表移除
>
...
上述 2026-07-22 待办已在后续 PBE 验证与正式服提升中完成；当前状态见下节。

## 12. 2026-08-27 正式服提升结果

- Riot 18.1 公告确认 S18“魔法森林”上线；`set18-live` 已成为默认且可查询的正式服空间，`set17-live` 按公告继续保留可选。
- `set18-pbe` 已从公开 SeasonContext 注册表移除，不再出现在普通用户的赛季选择、默认偏好或查询入口中。
- S18 正式服绑定 `queue=1100`、`TFTSet18`、`latest/zh_cn` lookup 和 18.1 公告；目录、缓存、会话与语义索引继续使用 `set18-live` 隔离键。
- 旧 SQLite 数据迁移继续显式归入 `set17-live`，不会因为默认空间切到 S18 而被重新标记。
- 保留底层 PBE provider 与内容提升契约作为历史兼容和未来赛季预览能力，但它们不构成可见或可查询的 S18 PBE 赛季空间。
