...
- 自动推导：根据阵容、升级路线和 Roll Timing 推导 `strategy`（例如 `reroll`、`fast8`、`fast9`），保存推导依据和算法版本，管理端只读展示。
