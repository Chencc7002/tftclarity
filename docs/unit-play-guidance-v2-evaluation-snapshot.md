...
- Formal Provider calls remain locked in `config.v2.json`.
...
Before any paid run, verify this snapshot commit is clean, rerun the zero-call preflight at that commit, and create a separate call-enabled commit only after explicit user authorization. Formal results still require two independent reviewers and adjudication of disagreements before any production decision.
